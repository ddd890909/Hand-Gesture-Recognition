...
void CompressiveTracker::getFeatureValue_ddd(Mat& _imageIntegral, Rect& _sampleBox_ddd, Mat& _sampleFeatureValue_ddd)
{
	...
	for (int i=0; i<featureNum; i++)
	{
		...
		for (size_t k=0; k<features[i].size(); k++)
		{
			//features中保存的特征模板（矩形框）是相对于box的相对位置的，  
            //所以需要加上box的坐标才是其在整幅图像中的坐标 
			xMin = _sampleBox_ddd.x + features[i][k].x;
			xMax = _sampleBox_ddd.x + features[i][k].x + features[i][k].width;
			yMin = _sampleBox_ddd.y + features[i][k].y;
			yMax = _sampleBox_ddd.y + features[i][k].y + features[i][k].height;
			//通过积分图来快速计算一个矩形框的像素和 
            //那么这里tempValue就是经过稀释矩阵加权后的灰度和了。  
            //每一个harr特征是由2到3个矩形框来构成的，对这些矩形框的灰度加权求和  
            //作为这一个harr特征的特征值。然后一个样本有50个harr特征 
		...
		_sampleFeatureValue_ddd.at<float>(i,1) = tempValue;	
	}
}
